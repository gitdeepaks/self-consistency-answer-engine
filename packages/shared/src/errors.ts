...
export function errorFacts(value: unknown): ErrorFacts {
  ...
  const shape = httpErrorShapeSchema.safeParse(error);
  const status = shape.success
    ? (shape.data.statusCode ?? shape.data.status ?? null)
    : null;
...
export function describeError(error: unknown): string {
  ...
  if (facts.name === "AbortError" || facts.name === "TimeoutError") {
    return facts.status === null
      ? "Timed out"
      : `HTTP ${facts.status}: timed out`;
  }

  const line =
    facts.status === null
      ? facts.message
      : `HTTP ${facts.status}: ${facts.message}`;
  return line.length > MAX_MESSAGE_LENGTH
    ? `${line.slice(0, MAX_MESSAGE_LENGTH - 3)}...`
    : line;
}
