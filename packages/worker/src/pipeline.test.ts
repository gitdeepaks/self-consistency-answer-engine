...
const behaviours: Record<ProviderId, Behaviour> = {
  openai: "ok",
  anthropic: "ok",
  google: "ok",
};
...
function candidateModel(id: ProviderId) {
  return new MockLanguageModelV4({
    ...
    doStream: async ({ abortSignal }) => {
      ...
      if (behaviour === "empty")
        return { stream: toStream(streamParts("   ")) };
      if (behaviour === "slow") {
...
const SPECS: Record<
  ProviderId,
  { id: ProviderId; label: string; color: string }
> = {
...
async function runFlow(runId: string): Promise<void> {
  ...
  await processSynthesisJob(
    { tenantId, runId },
    localJobMeta(`synth:${runId}`),
  );
}
...
describe("pipeline", () => {
  ...
  test("streams deltas without writing them to the durable log", async () => {
    ...
    const reader = (async () => {
      for await (const message of bus.subscribe(tenantId, seeded.id, {
        ...
        if (
          message.kind === "event" &&
          message.frame.event.type === "run.completed"
        )
          return;
      }
    })();
    ...
    const joined = deltas
      .map((event) => (event.type === "candidate.delta" ? event.text : ""))
      .join("");
  ...
  test("survives a partial panel failure and still synthesises", async () => {
    ...
    evaluatorPayload = {
      ...GOOD_SYNTHESIS,
      reviews: [GOOD_SYNTHESIS.reviews[0]],
    };
    ...
    const byProvider = Object.fromEntries(
      (run?.candidates ?? []).map((c) => [c.provider, c]),
    );
  ...
  test("honours a provider subset", async () => {
    evaluatorPayload = {
      ...GOOD_SYNTHESIS,
      reviews: [GOOD_SYNTHESIS.reviews[1]],
    };

    const seeded = await seedRun({
      prompt: "Only Claude please",
      providers: ["anthropic"],
    });
    await runFlow(seeded.id);
  ...
  test("backfills a review for any candidate the evaluator ignored", async () => {
    ...
    expect(run?.synthesis?.reviews[2]?.weaknesses[0]).toContain(
...
describe("redelivery", () => {
  ...
  test("a synthesis job delivered twice does not rewrite a finished run", async () => {
    ...
    await processSynthesisJob(
      { tenantId, runId: seeded.id },
      localJobMeta("synth:1"),
    );
...
describe("budgets", () => {
  ...
  test("a run that has spent its token ceiling stops fanning out", async () => {
    ...
    const run = await getRun(tenantId, seeded.id);
    expect(run?.candidates[0]?.status).toBe("OK");
    expect(run?.candidates.slice(1).every((c) => c.status === "SKIPPED")).toBe(
      true,
    );
