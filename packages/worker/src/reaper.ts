...
export async function reapOnce(now: Date = new Date()): Promise<number> {
  const overdue = await listOverdueRuns({
    scope: { kind: "every-tenant", reason: "deadline reaper runs across the whole install" },
    now,
    limit: 100,
  })
  ...
  for (const run of overdue) {
    ...
    try {
      ...
      console.error("[reaper] failed to reap run", { runId: run.id, error: describeError(error) })
    }
  }

  if (reaped > 0) console.warn(`[reaper] failed ${reaped} run(s) past their deadline`)
  return reaped
}
